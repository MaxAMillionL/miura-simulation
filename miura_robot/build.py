...
def load_config(path=None):
    ...
    for key in ['units_to_meters','timestep','panel_mass_kg','motor_mass_kg',
                ...
                'command_timeout_s','wheel_friction','fold_position_gain','fold_speed_limit_rad_s',
                'fold_integral_gain','fold_integral_limit_rad_s','drive_linear_speed_m_s','drive_yaw_speed_rad_s']:
        if not math.isfinite(c[key]) or c[key]<=0:
            raise ValueError(f'{key} must be finite and positive')
    if c['fold_speed_limit_rad_s']>c['motor_speed_limit_rad_s']:
        raise ValueError('fold_speed_limit_rad_s must not exceed motor_speed_limit_rad_s')
...
def layout(c):
    """Align the original connector profiles directly, without added spacers.

    All four panels face the same side in the flat state. Swapping panel 2's
    two edge assignments fixes the previous reflected, incompatible layout.
    Their ~12 mm connector envelopes overlap while the flat solids interleave.
    """
    cal=c['cad_calibration']
    length=cal['connector_edge_length_stl_units']
    origin=np.array(cal['connector_vertex_stl_units'])
    a=np.array([0.,-length,0]); b=np.array([length*np.sqrt(3)/2,-length/2,0])
    origins=[origin,origin+b,origin+a,origin]
    src_edges=[(a,b),(-b,a),(b,-a),(a,b)]
...
def split_rotating_connectors(frame,axes,centers,c):
    """Move each original male coupling off the frame and onto its output shaft."""
    cutters=[]; connectors=[]
    for center,axis in zip(centers,axes):
        inward=np.array([axis[1],-axis[0],0.])
        shaft=center-inward*2*c['cad_calibration']['gear_pitch_stl_units']
        cutter=trimesh.creation.cylinder(radius=5.1,height=6.502,sections=64)
        rr=Rotation.align_vectors([axis],[[0,0,1]])[0].as_matrix()
        cutter.vertices=cutter.vertices@rr.T+shaft+axis*17.251
        connectors.append(trimesh.boolean.intersection([frame,cutter],engine='manifold'))
        cutters.append(cutter)
    return trimesh.boolean.difference([frame,*cutters],engine='manifold'),connectors
...
def build(config=None):
    ...
    frame,rotating_connectors=split_rotating_connectors(frame,axes,centers,c)
    origins,rotations,rays,length=layout(c)
    # Separate ground contact from inter-panel contact. A whole convex frame
    # hull would fill the connector recesses and force the mating panels apart.
    inset=8.2
    core_origin=np.array(cal['connector_vertex_stl_units'])+[inset,-np.sqrt(3)*inset,0]
    core_length=cal['connector_edge_length_stl_units']-4*inset/np.sqrt(3)
    ca=np.array([0,-core_length,0]); cb=np.array([core_length*np.sqrt(3)/2,-core_length/2,0])
    points=[core_origin+v+[0,0,z] for v in [np.zeros(3),ca,cb,ca+cb] for z in [-6.001,6.001]]
    prism=trimesh.convex.convex_hull(np.array(points))
    core=trimesh.boolean.intersection([frame,prism],engine='manifold').convex_hull
    ...
    def mesh(name,m):
        # Preserve CAD Boolean topology without STL float32 quantization.
        m.export(gen/(name+'.obj'),digits=17)
        (gen/(name+'.stl')).unlink(missing_ok=True)
    ...
    ET.SubElement(world,'geom',name='floor',type='plane',size='2 2 .1',rgba='.16 .19 .24 1',contype='1',conaffinity='0')
    ...
    for p,body in enumerate(panels):
        ...
        # Ground hull retains the full footprint. The inset core handles
        # panel self-contact; original interlocking tabs use ideal hinge motion.
        ET.SubElement(body,'geom',name=f'panel_ground_{p}',type='mesh',mesh=f'frame_{p}',mass='0',group='3',rgba='1 0 0 0',contype='0',conaffinity='1')
        mesh(f'core_{p}',convert(core,origin,r))
        core_contact=dict(contact); core_contact['conaffinity']=str(30^bit)
        ET.SubElement(body,'geom',name=f'panel_collision_{p}',type='mesh',mesh=f'core_{p}',mass='0',group='3',rgba='1 0 0 0',**core_contact)
        for e,(g,axis_src,center) in enumerate(zip(gears,axes,centers)):
            ...
            for stage,suffix in enumerate(['','_idler','_output']):
                ...
                if stage==2:
                    connector_mesh=name+'_original_coupler'
                    mesh(connector_mesh,convert(rotating_connectors[e],source_pos,r))
                    ET.SubElement(drive,'geom',name=name+'_coupler',type='mesh',mesh=connector_mesh,rgba=colors[p],mass='.0003',contype='0',conaffinity='0')
            ...
            # Invisible reference sites identify the original mating profiles.
            # No connector, sleeve, rod or spacer geometry is added.
            for station in cal['connector_stations_stl_units']:
                ...
                for crease in [p,(p+1)%4]:
                    ray=rays[crease]
                    if np.linalg.norm(connector-ray*np.dot(connector,ray))<1e-6:
                        site=f'interlock_{p}_{e}_{station:g}'
                        ET.SubElement(body,'site',name=site,pos=vec(connector),size='.001',rgba='0 0 0 0',group='4')
                        mounts.append(dict(panel=p,edge=e,crease=crease,site=site,center_m=connector.tolist(),axis=axis.tolist()))
    ...
    couplings=[]
    if c.get('shaft_drives_interlock',False):
        for p,e,crease in sorted({(v['panel'],v['edge'],v['crease']) for v in mounts}):
            name=f'p{p}_m{e}'
            # On this Miura branch the fourth relative crease rotation is
            # -fold_2 in the corresponding motor shaft convention.
            joint=f'fold_{crease if crease else 2}'
            sign=1 if crease else -1
            ET.SubElement(equality,'joint',name=name+'_panel_coupling',joint1=name+'_output',joint2=joint,polycoef=f'0 {sign} 0 0 0',solref='.002 1')
            couplings.append(dict(motor=name,fold_joint=joint,sign=sign,crease=crease))
    ...
    if len(mounts)!=16: raise ValueError(f'Expected two original interlocks on each of eight joined panel edges, got {len(mounts)}')
    path=out/'robot.xml'; ET.indent(root); ET.ElementTree(root).write(path,encoding='unicode')
    metadata=dict(config=c,motors=manifest,interlock_sites=mounts,fold_couplings=couplings,initial_fold=folded_angles(c['initial_fold_rad']),crease_rays=[v.tolist() for v in rays],inferred_edge_length_m=length*scale)
